...
/** Soft preference for nav/catalog chunks — PAGE_LINKS only (no other path impact). */
function navigationSourceBonus(match, subIntent) {
  ...
  if (entityType === "listing") return 0.25;
  if (
    /\b(category|categories|collection|catalog|catalogue|navigation|menu)\b/.test(
      title,
    )
  ) {
    return 0.15;
