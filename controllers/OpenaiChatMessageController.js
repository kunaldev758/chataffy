...
const generalInquiryResponse = async (message, userId, chatMessageId) => {
  ...
  */
  try {
    const start = Date.now();
    const queryEmbedding = await OpenAIController.createEmbedding(message);
    const queryEmbeddingValues = queryEmbedding[0].embedding;
  ...
  const pipeline = [
    ...
    {
      $unwind: '$mappings', // Unwind the parts array to treat each part separately
    },
    {
      $project: {
        // 'webPage.content': 1,
        ...
        'embedding': '$mappings.embeddingValues',
      },
    },
    {
      $addFields: {
        similarity: {
          $function: {
            ...
            `,
            args: [queryEmbeddingValues, '$embedding'],
            lang: 'js',
          },
        },
      },
  ...
  const data = {
    ...
    sources: similarTrainingLists.map(trainingList => trainingList._id)
  };
  return data;
  }
  catch(error) {
...
const chat_message_response = async (chatMessage, visitor_id, conversation_id, socket_io, userId) => { 
  ...
  const trainingList = await TrainingList.aggregate([
    {
      $match: {
        userId,
        type: 0,
        'trainingProcessStatus.mappingStatus': 2,
        'webPage.url': { $exists: true, $ne: null }
      }
    },
    {
      $addFields: {
        urlLength: { $strLenCP: "$webPage.url" }
      }
    },
    {
      $sort: {
  ...
  const organisation = trainingList[0].webPage.title+": "+trainingList[0].webPage.metaDescription;
...
Note: Understand the thing that visitor wants to ask from organization; not from you as any individual. Visitor is considering you AI based chat agent from the organisation. Beware of any confusion you may have for telling about yourself and telling about organization.
...
"greeting" - Use this intent for messages containing only greetings or initial salutations.
        ...
        for (const enquiry of arguments.enquiries) {
          switch(enquiry.intent)
          {
            ...
            case "general_enquiry":
              ...
              relevant_information = related_data.contents; 
...
for (const enquiry of enquiries) { // enquiry.potential_information.faqs || 
  if(enquiry.enquiry_guidelines) {
    ...
    for (const content of enquiry.potential_information) {
      information_based_enquiries += `
...
Craft a final chat_reply for visitor_message by the logical combination of all the reply_text from enquiry_replies, without using any other knowledge. Ensure the response is as small as possible while addressing all relevant aspects of visitor_message. Use a friendly tone and concise language to provide a seamless chat experience. Do not create any email, url, contact or any information by your own; which are not mentioned in provided information.
...
- Represent the organization's perspective using pronouns like 'We, I, us' when responding, without explicit mention.
- Understand the thing that visitor wants to ask from organization; not from you as any individual. Beware of any confusion you may have for telling about yourself and telling about organization.
- Prioritizes the organization's interests, maintains a professional tone. 
- Includes emojis for engagement. 
- The response should be formatted in HTML with lists, hyperlinked URLs, and "mailto:" protocol for email addresses, very short paragraphs.
- Send a follow-up message offering assistance if the initial message contains only a greeting and no specific enquiry.
...
Ensure to follow these steps carefully to achieve accurate results for the given task.
`},
// You are not answerable for all the questions. You can politely deny, if no relevant information found for any enquiry of visitor. 
{role: "user", content:`
<visitor_message>
${message}
</visitor_message>
...
${information_based_enquiries && `
<information_based_enquiries>
<note>These enquiries must be answered using associated contents only. You can deny politely rather than saying anything which is not given in associated content.</note>
${information_based_enquiries}
</information_based_enquiries>`}
${previous_conversation && `
<previous_conversation>
