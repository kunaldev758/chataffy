...
const myMiddleware = async (socket, next) => {
  ...
  const isUserConnection = socket.handshake.query.token !== undefined && socket.handshake.query.embedType !== undefined;
  const isVisitorConnection = socket.handshake.query.widgetId !== undefined && socket.handshake.query.widgetAuthToken !== undefined && socket.handshake.query.embedType !== undefined;
  try {
    if (isUserConnection) {
      ...
      try { 
        if(socket.handshake.query.visitorId!== undefined) {
          ...
          if(!visitor || visitor.userId.toString() != socket.userId.toString())
          {
      ...
      throw new Error("Invalid connection type. Please provide valid credentials.");
    }
    next();
  } catch (error) {
...
SocketController.handleSocketEvents = (io) => {
  ...
  io.on("connection", (socket) => {  
    if(socket.type == "client") {
      ...
      /*----------*/
      if(socket.embedType=='tensorflow') {
        console.log("client using tensorflow");
        ...
        socket.on("get-training-list-count", async (data) => {
          const userId = socket.userId;
          const webPagesCount = await TensorflowTrainingListController.getWebPageUrlCount(userId);
          const docSnippets = await TensorflowTrainingListController.getSnippetCount(userId);
          // {crawledDocs: 0, totalDocs: 0};
          const faqs = await TensorflowTrainingListController.getFaqCount(userId);
        ...
        socket.on("get-training-list", async (data) => {
          const userId = socket.userId;
          const webPages = await TensorflowTrainingListController.getWebPageList(userId);
      ...
      if(socket.embedType=='openai') {
        ...
        socket.on("get-conversations-list", async (data) => {
          const userId = socket.userId;
          const visitors = await Visitor.find({userId, lastMessage: { $exists: true } }).sort({createdAt: -1});
          socket.emit("get-conversations-list-response", {
            response: "Received data from message",
            data: visitors,
          });
    ...
    if(socket.type == "visitor") {
      ...
      if(socket.embedType=='tensorflow') {
        socket.on("visitor-connect", async (data) => {
          // console.log("check visitor connect");
          try {
            let visitorId=socket.visitorId, chatMessages=[];          
            conversationId = visitorId;
            // console.log(visitorId, conversationId);
            chatMessages = await TensorflowChatMessageController.getAllChatMessages(conversationId);
            console.log("Messages", chatMessages);
            if(!chatMessages.length) {
              const chatMessage = await TensorflowChatMessageController.createChatMessage(conversationId, visitorId, 'system', "Conversation start");
        ...
        socket.on('visitor-send-message', async (data, callback) => {
          // console.log('visitor-send-message', data);
          const { message, id } = data;
          // console.log("received message",data);
          const visitorId = socket.visitorId;
          conversationId = visitorId;
          const encodedMessage = encode(message);
          let response_data, chatMessage=null;
          // console.log('encodedMessage',encodedMessage);
          try {
            chatMessage = await TensorflowChatMessageController.createChatMessage(conversationId, visitorId, 'visitor', "<p>"+encodedMessage+"</p>");
            // socket.emit("visitor-send-message-response",{"chat_message":chatMessage});
            io.to("conversation"+socket.conversationId).emit('conversation-append-message',{"chatMessage":chatMessage, id});
          } catch (error) {
          ...
          try {
            response_data = await TensorflowChatMessageController.chat_message_response(chatMessage, socket.visitorId, socket.conversationId, io, socket.userId);
          ...
          try {
            ...
            io.to("visitor"+socket.visitorId).emit('intermediate-response', {"message":false});
            ...
            if(response_data.error) {
              ...
              const chatMessageResponse = await TensorflowChatMessageController.createChatMessage(conversationId, visitorId, 'bot-error', "Error in response");
              io.to("conversation"+socket.conversationId).emit('conversation-append-message', {"chatMessage":chatMessageResponse});
              // response_data.error
            }
            else {
              const chatMessageResponse = await TensorflowChatMessageController.createChatMessage(conversationId, visitorId, 'bot', response_data.reply, response_data.infoSources);
              io.to("conversation"+socket.conversationId).emit('conversation-append-message', {"chatMessage":chatMessageResponse, sources: response_data.sources });
      ...
      if(socket.embedType=='openai') {
        socket.on("visitor-connect", async (data) => {
          ...
          try {
            ...
            // console.log("Messages", chatMessages);
            if(!chatMessages.length) {
              const chatMessage = await OpenaiChatMessageController.createChatMessage(conversationId, visitorId, 'system', "Conversation start");
              chatMessages = [chatMessage];
            }    
        ...
        socket.on('visitor-send-message', async (data, callback) => {
          ...
          // console.log('encodedMessage',encodedMessage);
          try {
            chatMessage = await OpenaiChatMessageController.createChatMessage(conversationId, visitorId, 'visitor', "<p>"+encodedMessage+"</p>");
            // socket.emit("visitor-send-message-response",{"chat_message":chatMessage});
            io.to("conversation"+socket.conversationId).emit('conversation-append-message',{"chatMessage":chatMessage, id});
          } catch (error) {
            console.log('visitor-send-message-error');
          ...
          try {
            response_data = await OpenaiChatMessageController.chat_message_response(chatMessage, socket.visitorId, socket.conversationId, io, socket.userId);
            // console.log("response_data", response_data);
          } catch (error) {
            console.log('newChatMessageError');
            socket.emit('newChatMessageError', error.message);
          ...
          try {
            ...
            if(response_data.error) {
              // io.to("visitor"+socket.visitorId).emit('chat-response-error', {"message_for": "abcd", "error": "Error in response"});
              const chatMessageResponse = await OpenaiChatMessageController.createChatMessage(conversationId, visitorId, 'bot-error', "Error in response");
              io.to("conversation"+socket.conversationId).emit('conversation-append-message', {"chatMessage":chatMessageResponse});
              // response_data.error
            }
            else {
              const chatMessageResponse = await OpenaiChatMessageController.createChatMessage(conversationId, visitorId, 'bot', response_data.reply, response_data.infoSources);
              io.to("conversation"+socket.conversationId).emit('conversation-append-message', {"chatMessage":chatMessageResponse, sources: response_data.sources });
              // io.to(conversationId).emit('newChatMessage', chatMessage);
            }
