...
- MAIN CATALOG / linked products only: show labeled fields in this order: <strong>Name:</strong>, then <strong>Price:</strong> only when a price exists, then <strong>Link:</strong>.
...
- Include a product in the main Name/Price/Link list ONLY when the knowledge base has a real Product URL for it.
- If a price is missing, omit the Price field entirely — do not write "Not listed", "N/A", or any placeholder.
- If a URL is missing, omit that product from the main list entirely — never write <strong>Link:</strong> Not available; never invent a URL.
- Products without a URL may still be mentioned briefly by name (and price if present) as soft suggestions only — no Link field.
...
- Main navigational list: include ONLY products that have a real Product URL (MAIN CATALOG), up to ${countHint} if a number was requested
- Start with one concise <h1>, add a useful <h2>, and wrap linked products in <ul>/<li>
- Each main-list product must show labeled Name and Link; include Price only when present
- If a price is missing, omit Price entirely — never write "Not listed" or placeholders
- Never write "Link: Not available"; never invent URLs. Products without a URL must not appear in the main list
- SUGGESTION-ONLY / unlinked products may be mentioned briefly by name (and price if present) as soft suggestions after the main list
...
- Do not invent products, sizes, or URLs; do not reference "the context" or "the provided context" in your response`;
    ...
    if (wantsProductUrls) {
      instructions += `
- **CRITICAL**: User asked for URLs — every product in the main list MUST include its URL from the knowledge base
...
- For products with a real URL, show labeled Name, Price (only when present), and Link. If a price is missing, omit Price — never write "Not listed".
- If a product has no URL, discuss it without a Link field; never write "Not available" or invent a URL.
...
- Use <ul>/<li> for trade-offs. For products with a real URL, show Name, Price (only when present), and Link.
- If a price is missing, omit Price entirely — never write "Not listed". If a URL is missing, omit Link entirely — never write "Not available"; never invent either value.
...
- If products are listed navigationally, include ONLY products with a real URL; show Name, Price (only when present), and Link.
- If a price is missing, omit Price entirely. If a URL is missing, omit that product from the main list (you may mention it as a soft suggestion by name only). Never write "Not listed" / "Not available"; never invent either value.
