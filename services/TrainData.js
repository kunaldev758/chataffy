...
function sendClientEvent(userId, eventName, data) {
  // If you have a global io instance
  if (global.io) {
    global.io.to(`user${userId}`).emit(eventName, data);
  }
  emitEvent(eventName, data, `user-${userId}`);
}
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // Calculate costs
      const totalTokens = await pricingCalculator.estimateTokens(content);
      const embeddingCost = await pricingCalculator.calculateEmbeddingCost(
        totalTokens
      );
      ...
      //   sendClientEvent(userId, "web-page-status", {
      //     trainingListId,
      ...
      sendClientEvent(userId, "web-page-status", {
        trainingListId,
        message: "Upserting vectors to database",
      });
      ...
      // req.io
      //   .to("user" + trainingListObj.userId)
      //   .emit("web-page-minifying-failed", {
