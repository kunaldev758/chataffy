...
const processWebPage = async (url, sourceCode, footerCache = {}) => {
  try {
    ...
    // ---- Clean whitespace ----
    const cleanContent = markdown.replace(/\s+/g, " ").trim();
...
//         // Extract footer links and add to a separate section
//         const footerLinks = [];
//         footer.find("a").each((_, el) => {
//           const href = $(el).attr("href");
//           const text = $(el).text().trim();
//           if (href) {
//             const absoluteUrl = href.startsWith("http") ? href : urlModule.resolve(webPageURL, href);
//             footerLinks.push(`${text || href}: ${absoluteUrl}`);
//           }
//         });
        
//         // Create a markdown list of footer links
...
//       if (href && text) {
//         const absoluteUrl = href.startsWith("http") ? href : urlModule.resolve(webPageURL, href);
//         navLinks.push(`${text}: ${absoluteUrl}`);
//       }
//     });
    
//     // Create a markdown list of navigation links
//     let navLinksList = "";
...
//         // We're not adding to allLinks array since we're keeping the original format
//         // but the links in the content will be absolute now
//       }
//     });
...
//       if (src) {
//         const absoluteSrc = src.startsWith("http") ? src : urlModule.resolve(webPageURL, src);
//         const description = alt || title || "";
//         const altText = description ? ` (${description})` : "";
        
//         $(el).replaceWith(`<p>Image${altText}: ${absoluteSrc}</p>`);
//         imagesList.push(`${description || "Unnamed image"}: ${absoluteSrc}`);
//       }
//     });
    
//     // Create a markdown list of all images
//     let imagesListMarkdown = "";
...
//     // Add our extracted links and images sections
//     let appendices = "";
//     appendices += navLinksList;
//     appendices += footerLinksList;
//     appendices += imagesListMarkdown;
    
//     // Add structured data if present
//     const structuredDataSections = [];
//     $('script[type="application/ld+json"]').each((_, el) => {
//       try {
//         const jsonData = JSON.parse($(el).html());
//         const type = jsonData["@type"] || "Unknown";
//         structuredDataSections.push(`### Structured Data (${type})\n\`\`\`json\n${JSON.stringify(jsonData, null, 2)}\n\`\`\``);
//       } catch (e) {
//         // Invalid JSON, skip
//       }
//     });
    
//     if (structuredDataSections.length > 0) {
//       appendices += "\n\n## Structured Data\n" + structuredDataSections.join("\n\n");
//     }

//     // ---- Clean whitespace ----
//     const cleanContent = (markdown + appendices)
